...
namespace abldeployer.Core {
    ...
    public class MultiCompilation {
        ...
        /// </summary>
        public int CurrentNumberOfProcesses {
        ...
        public void CompileFiles(List<FileToCompile> filesToCompile) {
            ...
            foreach (var proExecutionCompile in _processes) proExecutionCompile.Start();
        }
        ...
        public void KillProcesses() {
            _hasBeenKilled = true;
            if (_processesRunning > 0) foreach (var proc in _processes.Where(proc => proc != null)) proc.KillProcess();
            _processesRunning = 0;
        }
        ...
        private void EndOfCompilation() {
            ...
            if (_processesRunning > 0)
                return;
            TotalCompilationTime = TimeSpan.FromMilliseconds(DateTime.Now.Subtract(StartingTime).TotalMilliseconds);

            // add all found handled exceptions
            foreach (var exception in _processes.Where(compile => compile.HandledExceptions != null).SelectMany(compile => compile.HandledExceptions)) AddHandledExceptions(exception);
        ...
        /// </summary>
        private void DoInLock(Action toDo) {
            Monitor.Enter(_lock);
            try {
                toDo();
            } catch (Exception e) {
        ...
        private void AddHandledExceptions(Exception exception, string customMessage = null) {
            Monitor.Enter(_lockErrors);
            try {
                if (HandledExceptions == null)
                    HandledExceptions = new List<MultiCompilationException>();
                if (customMessage != null)
                    HandledExceptions.Add(new MultiCompilationException(customMessage, exception));
                else HandledExceptions.Add(new MultiCompilationException("ABL Compilation exception", exception));
            } finally {
