...
namespace csdeployer.Core {
    public class Config {
        ...
        private static ProConfig _currentEnv;
        ...
        /// </summary>
        public static ProConfig Instance {
            get {
                if (_currentEnv == null) {
                    _currentEnv = new ProConfig();
                }
                return _currentEnv;
            }
        ...
        public static ProConfig Load(string path) {
            XmlSerializer serializer = new XmlSerializer(typeof(ProConfig));
            using (StreamReader reader = new StreamReader(path)) {
                return (ProConfig) serializer.Deserialize(reader);
            }
        }

        public static void Save(ProConfig config, string path) {
            XmlSerializer serializer = new XmlSerializer(typeof(ProConfig));
        ...
        public class ProConfig {
            ...
            internal string FolderTemp {
                get { return _folderTemp ?? (_folderTemp = CreateDirectory(Path.Combine(!string.IsNullOrEmpty(OverloadFolderTemp) ? OverloadFolderTemp : Path.Combine(Path.GetTempPath(), AssemblyInfo.AssemblyProduct), Path.GetRandomFileName()))); }
            }
            ...
            /// </summary>
            internal string RuleErrors {
                get { return _ruleErrors; }
            }
