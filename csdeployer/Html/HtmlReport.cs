        ...
        public static void ExportReport(DeploymentHandlerDifferential deployment, string path) {
            ...
            html.AppendLine(@"
                <table class='ToolTipName' style='margin-bottom: 0px; width: 100%'>
                    ...
                    <tr>");
            switch (CsConfigDeploymentPackaging.Instance.ReturnCode) {
            ...
            if (!string.IsNullOrEmpty(CsConfigDeploymentPackaging.Instance.RaisedException)) {
                html.AppendLine(@"<h2>Problèmes rencontrés pendant le déploiement :</h2>");
                html.AppendLine(@"<div class='IndentDiv errors'>");
                html.AppendLine(CsConfigDeploymentPackaging.Instance.RaisedException);
                html.AppendLine(@"</div>");
                html.AppendLine(@"<div class='IndentDiv'>");
                html.AppendLine(@"<b>Lien vers le fichier .log pour plus de détails :</b><br>");
                html.AppendLine(CsConfigDeploymentPackaging.Instance.ErrorLogFilePath.ToHtmlLink());
                html.AppendLine(@"</div>");
            }

            if (CsConfigDeploymentPackaging.Instance.RuleErrors != null && CsConfigDeploymentPackaging.Instance.RuleErrors.Count > 0) {
                html.AppendLine(@"<h2>Erreurs de règles :</h2>");
                html.AppendLine(@"<div class='IndentDiv errors'>");
                foreach (var fileErrors in CsConfigDeploymentPackaging.Instance.RuleErrors) {
            ...
            ");
                if (sourceFilesNotDeployed.Count > 0) {
                    ...
                    foreach (var undeployedFile in sourceFilesNotDeployed) {
                        currentReport.AppendLine(@"<div><img height='15px' src='" + Utils.GetExtensionImage((Path.GetExtension(undeployedFile) ?? "").Replace(".", "")) + "'>" + Path.Combine(deployment1.ProEnv.SourceDirectory, undeployedFile).ToHtmlLink(undeployedFile, true) + @"</div>");
                    }
                    currentReport.AppendLine(@"</div>");
                }
                currentReport.AppendLine(@"</div>");
