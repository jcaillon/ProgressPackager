...
namespace csdeployer.Core.Deploy {
    ...
    internal class MultiCompilation {
        ...
        public List<FileToCompile> GetListOfFileToCompile {
            get { return (_processes ?? new List<ProExecutionCompile>()).SelectMany(proc => { return proc.Files != null && proc.NbFilesTreated > 0 ? proc.Files.GetRange(0, proc.NbFilesTreated) : new List<FileToCompile>(); }).ToList(); }
        }
