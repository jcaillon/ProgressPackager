    ...
    internal static class Utils {
        ...
        public static void ForEachLine(string filePath, byte[] dataResources, Action<int, string> toApplyOnEachLine, Encoding encoding = null) {
            try {
                ...
                if (!ForEachLine(filePath, dataResources, toApplyOnEachLine, encoding ?? TextEncodingDetect.GetFileEncoding(filePath), exception => ex = exception)) {
                    ErrorHandler.LogErrors(ex, "Impossible de lire le fichier " + filePath.Quoter());
                }
            } catch (Exception e) {
                ErrorHandler.LogErrors(e, "Impossible de lire une ressource interne");
            }
        }
        ...
        public static string ReadAllText(string path, Encoding encoding = null) {
            try {
                ...
                ErrorHandler.LogErrors(e, "Impossible de lire le fichier " + path.Quoter());
            }
            return null;
        }
