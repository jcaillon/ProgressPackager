...
namespace csdeployer.Lib.Compression.Prolib {
    /// <summary>
    ...
    internal class ProlibPackager : IPackager {
        ...
        public void PackFileSet(IDictionary<string, FileToDeployInPack> files, CompressionLevel compLevel, EventHandler<ArchiveProgressEventArgs> progressHandler) {
            ...
            if (string.IsNullOrEmpty(archiveFolder)) {
                throw new Exception("Impossible de trouver le dossier de l'archive cible");
            }
                ...
                Parallel.ForEach(subFolder.Value, file => {
                    ...
                    try {
                        if (file.Move) {
                            ...
                            throw new Exception("Impossible de trouver le fichier temporaire pour " + file.Origin);
                        }
                    } catch (Exception e) {
                        ex = e;
                    }
